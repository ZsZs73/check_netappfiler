...
def SNMPGET_cmdline(oid):
	cmdline = CMDLINE_get % oid
	...
	out = cmd.readline().rstrip()
	retcode = cmd.close()

	if retcode == 256:
		print 'SNMP UNKNOWN: Timeout - no SNMP answer from %s' % options.host
		sys.exit(RETURNCODE['UNKNOWN'])
	if retcode == 512:
		print 'SNMP UNKNOWN: OID (%s) not found' % oid
		sys.exit(RETURNCODE['UNKNOWN'])
	elif retcode != None:
		print 'SNMP UNKNOWN: Unknown error code %s' % retcode
		sys.exit(RETURNCODE['UNKNOWN'])
...
def SNMPWALK_cmdline(oid):
	cmdline = CMDLINE_walk % oid
...
def get_fsid(fsid):
	...
	if fsid.isdigit() == False:
		...
		if fsidcache.has_key(fsid):
			...
			dfFSName = SNMPGET(OIDs['df_FS_Name'] + "." + str(newindex))
			if dfFSName[-1] == '/':
...
def find_in_table(oid_index, oid_names, wanted):
	...
	if options.verb >= 2:
		print 'Tablefinder looked for "%s" in "%s", idx: %s' % (wanted, oid_names, index)
...
parser.add_option("", "--path",
		  dest="path",
		  help="Path to snmpget and snmpwalk")
parser.add_option("", "--nonetsnmp",
		  action="store_true",
		  dest="nonetsnmp",
...
if options.version not in ['1', '2', '2c', '3']:
	print "Unknown protocol version (-P) %s " % options.version
	sys.exit(127)

if options.version == '3':
	if options.seclevel not in ['AuthNoPriv', 'NoAuthNoPriv',]:
		print "Unknown security level (-L) %s!" % options.seclevel
		sys.exit(127)
	if options.authproto not in ['md5', 'sha',]:
...
if use_netsnmp:
	...
	SNMPGET		= SNMPGET_netsnmp
	SNMPWALK	= SNMPWALK_netsnmp

	if options.version == '2c':
		options.version = '2'
	...
	SNMPGET		= SNMPGET_cmdline
	SNMPWALK	= SNMPWALK_cmdline

	if options.version == '3':
		CMDLINE_get	= '%s/snmpget -v3 -OqevtU -l%s -u%s -a%s -A%s %s %%s 2>/dev/null' % (options.path, options.seclevel, options.secname, options.authproto, options.authkey, options.host)
		CMDLINE_walk	= '%s/snmpwalk -v3 -OqevtU -l%s -u%s -a%s -A%s %s %%s 2>/dev/null' % (options.path, options.seclevel, options.secname, options.authproto, options.authkey, options.host)
	else:
		CMDLINE_get	= '%s/snmpget -v%s -c%s -OqevtU %s %%s 2>/dev/null' % (options.path, options.version, options.community, options.host)
		CMDLINE_walk	= '%s/snmpwalk -v%s -c%s -OqevtU %s %%s 2>/dev/null' % (options.path, options.version, options.community, options.host)

	if options.verb >=3:
		print "Using commandline: " + CMDLINE_get
	...
	if SNMPGET(OIDs['License_SnapMirror']) != '2':
		...
		if SnapmirrorOn != 2:
			...
			if options.var == '0':
				...
				idx = find_in_table(OIDs['Snapmirror_Index'], OIDs['Snapmirror_Src'], options.var)
				if not idx:
					back2nagios(RETURNCODE['UNKNOWN'], 'No snapmirror with source "%s" found!' % options.var)

				SnapmirrorSrc		= SNMPGET(OIDs['Snapmirror_Src'] + "." + idx)
